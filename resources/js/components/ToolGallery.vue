<template>
  <div class="gallery" :class="{editable}" @mouseover="mouseOver = true" @mouseout="mouseOver = false">
    <cropper v-if="true && editable" :image="cropImage" :must-crop="field.mustCrop" @close="onCloseCroppedImage" @crop-completed="onCroppedImage" :configs="field.croppingConfigs"/>
    ...
    <span v-if="editable" class="form-file">
      ...
      <label :for="`__media__${field.attribute}`" v-if="images.length == 0" class="form-file-btn btn btn-default btn-primary">
        <span v-if="uploading">{{ __('Uploading') }} ({{ uploadProgress }}%)</span>
        <span v-else>{{ label }}</span>
      </label>
      <label v-if="images.length > 0" class="form-file-btn btn btn-default btn-primary" @click="doUpload">
        <span v-if="uploading">{{ __('Uploading') }} ({{ uploadProgress }}%)</span>
        <span v-else>Save To Library</span>
      </label>
    </span>

    <p v-if="uploadSuccess && images.length == 0" class="my-2">
      File saved!
    </p>

    <p v-if="uploadError" class="my-2 text-danger">
      {{ uploadError }}
    </p>
  </div>
</template>
